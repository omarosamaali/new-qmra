...
// ── HTTPS bounce: payment gateways redirect here, then user is sent to custom scheme (see SubscriptionsController::mobileReturn)
Route::get('/subscriptions/mobile-return', [SubscriptionsController::class, 'mobileReturn'])->name('subscriptions.mobile-return');
...
Route::get('/about', function () {
    ...
    try {
        ...
        if ($res->successful()) {
            $settings = $res->json('data') ?? $res->json();
        }
    } catch (Exception $e) {
    }
...
Route::middleware(['auth', 'subscribed'])->group(function () {
    ...
    Route::get('/add-vehicle', function (Request $req) {
        $userId = Auth::id() ?? 1;
        $count = Vehicle::where('user_id', $userId)->count();
        ...
        return Inertia::render('Phone/AddVehicle', [
            'vehicleCount' => $count,
            'carsLimit' => $limit,
        ]);
    });
...
Route::post('/api/car/generate-code', function () {
    ...
    Cache::put('car_code:'.$code, true, now()->addMinutes(10));
...
Route::get('/api/car/validate-code/{code}', function (string $code) {
    $valid = Cache::has('car_code:'.strtoupper($code));
...
Route::get('/api/car/vehicle/{linkCode}', function (string $linkCode) {
    $vehicle = Vehicle::where('link_code', strtoupper($linkCode))
...
Route::get('/api/car/reminders/{linkCode}', function (string $linkCode) {
    ...
    $reminders = Reminder::where('vehicle_id', $vehicle->id)
...
// Get all reminders (upcoming + completed) by link_code — used by car dashboard
Route::get('/api/car/reminders/{linkCode}/all', function (string $linkCode) {
    $vehicle = Vehicle::where('link_code', strtoupper($linkCode))
        ->where('is_linked', true)
        ->first();
    if (! $vehicle) {
